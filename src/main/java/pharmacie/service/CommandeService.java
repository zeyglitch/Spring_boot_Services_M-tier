...
public class CommandeService {
    ...
    public Ligne ajouterLigne(int commandeNum, int medicamentRef, @Positive int quantite) {
        log.info("Service : Ajout de {} unités du médicament {} à la commande {}", quantite, medicamentRef,
                commandeNum);

        // Vérifier que la commande existe
        var commande = commandeDao.findById(commandeNum).orElseThrow();

        // Vérifier que la commande n'a pas encore été envoyée
        ...
        if (ligneExistante.isPresent()) {
            ...
            int nouvelleQuantiteCommandeeMedicament = medicament.getUnitesCommandees() + quantite;
            if (medicament.getUnitesEnStock() < nouvelleQuantiteCommandeeMedicament) {
                throw new IllegalStateException("Stock insuffisant");
            }

            // Mettre à jour la ligne et le stock commandé
            ligne.setQuantite(nouvelleQuantite);
            medicament.setUnitesCommandees(nouvelleQuantiteCommandeeMedicament);
            ...
            // Créer et sauvegarder la nouvelle ligne
            var nouvelleLigne = new Ligne(commande, medicament, quantite);

            // Incrémenter les unités commandées
            medicament.setUnitesCommandees(nouvelleQuantiteCommandeeMedicament);
            medicamentDao.save(medicament);
    ...
    public void supprimerLigne(int id) {
        ...
        // Décrémenter les unités commandées du médicament
    ...
    public Commande enregistreExpedition(int commandeNum) {
        ...
        // Renseigner la date d'expédition avec la date du jour
        commande.setEnvoyeele(LocalDate.now());

        // Pour chaque ligne de la commande, décrémenter le stock et les unités
        // commandées
        for (var ligne : commande.getLignes()) {
            var medicament = ligne.getMedicament();
            int quantite = ligne.getQuantite();

            // Décrémenter les unités en stock
